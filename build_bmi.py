...
def title_page(doc):
    ...
    c("Mavzu: “Kuniga 4 tonna quvvatga ega tuzlangan bodring", bold=True, size=15, after=0)
    c("ishlab chiqarish texnologiyasini o‘rganish”", bold=True, size=15, after=50)
    # bajardi/rahbar
...
def main():
    ...
    title_page(doc)
    add_toc(doc)
    with open('bmi_content.md', encoding='utf-8') as f:
        md = f.read()
    parse_and_build(doc, md)
    footer_page_numbers(doc)
    enable_update_fields(doc)
    out = "Tuzlangan_bodring_BMI.docx"
    doc.save(out)
    # statistika
