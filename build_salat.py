...
def title_page(doc):
    ...
    c("Mavzu: “Aralash sabzavotlardan tayyorlanadigan salat", bold=True, size=15, after=0)
    c("konservalarining texnologiyasini ishlab chiqish”", bold=True, size=15, after=50)
    # bajardi/rahbar
...
def main():
    ...
    title_page(doc)
    add_toc(doc)
    with open('salat_content.md', encoding='utf-8') as f:
        md = f.read()
    parse_and_build(doc, md)
    footer_page_numbers(doc)
    enable_update_fields(doc)
    out = "Salat_konservalari_BMI.docx"
    doc.save(out)
    # statistika
