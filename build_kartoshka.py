...
def title_page(doc):
    ...
    c("Mavzu: “Kartoshka asosida quritilgan yarim tayyor", bold=True, size=15, after=0)
    c("konservalar ishlab chiqarish usullari”", bold=True, size=15, after=50)
    # bajardi/rahbar
...
def main():
    ...
    title_page(doc)
    add_toc(doc)
    with open('kartoshka_content.md', encoding='utf-8') as f:
        md = f.read()
    parse_and_build(doc, md)
    footer_page_numbers(doc)
    enable_update_fields(doc)
    out = "Kartoshka_quritilgan_BMI.docx"
    doc.save(out)
    # statistika
