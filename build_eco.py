...
def title_page(doc):
    ...
    c("Mavzu: “Mahalliy resurslardan foydalanib kunlik 9 t bo‘lgan", bold=True, size=14, after=0)
    c("ekologik toza konservalangan mahsulotlarni", bold=True, size=14, after=0)
    c("ishlab chiqarish texnologiyasi”", bold=True, size=14, after=50)
    # bajardi/rahbar
...
def main():
    ...
    title_page(doc)
    add_toc(doc)
    with open('eco_content.md', encoding='utf-8') as f:
        md = f.read()
    parse_and_build(doc, md)
    footer_page_numbers(doc)
    enable_update_fields(doc)
    out = "Ekologik_konservalar_BMI.docx"
    doc.save(out)
    # statistika
